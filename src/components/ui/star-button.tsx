...
export function StarButton({
    ...
    useEffect(() => {
        ...
        const updatePath = () => {
            ...
            if (!div) return;
            // Use clientWidth/Height to include padding but exclude border if using border-box, 
            // but offsetWidth is usually safer for border-box dimensions of the element itself.
            // For the path to trace the border exactly, we need the full outer size.
        ...
        // Observe changes for hover animations
        const observer = new ResizeObserver(updatePath);
        observer.observe(pathRef.current as Element);
    ...
    const classes = cn(
        "relative z-[3] overflow-hidden h-10 px-4 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 group/star-button hover:-translate-y-0.5",
        "bg-white/5 backdrop-blur-xl border border-white/10 shadow-lg", // Glassmorphism
        className,
    );
    ...
    const InnerContent = () => (
        <>
            ...
            {/* Fixed: Removed text-transparent bg-clip-text so icons use currentColor properly */}
            <span className="z-10 relative text-white/90 group-hover:text-white transition-colors inline-flex items-center gap-2">
                {children}
            </span>
        </>
    );
